// Moderate a LiveKit participant (mute / remove). Host-only, wallet-signed.
// Deploy: supabase functions deploy livekit-moderate --no-verify-jwt
...
Deno.serve(async (req) => {
  ...
  const { address, dbRoomId, livekitRoom, identity, action, message, signature } = body;
  if (!address || !dbRoomId || !livekitRoom || !identity || !action || !message || !signature) return json({ error: "missing fields" }, 400);
  if (action !== "mute" && action !== "remove") return json({ error: "bad action" }, 400);

  const prefix = `Midgard Live · mod · ${livekitRoom} · `;
  ...
  // Only the room's host may moderate.
  ...
  try {
    if (action === "remove") {
      ...
      const p = await svc.getParticipant(livekitRoom, identity);
      for (const t of p.tracks ?? []) {
        try { await svc.mutePublishedTrack(livekitRoom, identity, t.sid, true); } catch { /* skip */ }
      }
    }
    return json({ ok: true });
  } catch (e) {
    return json({ error: "moderation_failed", detail: String(e) }, 500);
  }
});
